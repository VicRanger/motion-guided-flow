...
def create_act_func(name="relu", out_channel=None):
    ops = {
        ...
        'lrelu': lambda: nn.LeakyReLU(negative_slope=0.1),
        'lrelu_': lambda: nn.LeakyReLU(negative_slope=0.1, inplace=True),
...
def create_norm_func(name="batch_norm_2d"):
    ops = {
        ...
        # 'instance_norm_2d': lambda x: torch.nn.LayerNorm(num_features=x),
    }
    return ops[name]
...
class NetBase(nn.Module):
    ...
    def return_output_by_type(self, ret):
        # log.debug(f'return_output_by_type: "{self.output_type}"')
...
class Outputable(NetBase):
    ...
    def get_out_channel(self, config: dict):
        ...
        if 'out_channel' in config.keys():
            ...
            get_channel = True
        if len(self.out_items) > 0:
            for item in self.out_items:
                self.out_channel += item['channel']
            get_channel = True
        assert get_channel, f'no "out_channel"(num) or "output"(array) specified in config: {config}'
