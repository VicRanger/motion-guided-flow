...
def norm_and_act(out_channel, act_func_name, norm_func_name, single_act_channel, seq=None, postfix=""):
    if seq is None:
        seq = nn.Sequential()
    if norm_func_name:
        seq.add_module(f"{norm_func_name}{postfix}",
                       create_norm_func(norm_func_name)(out_channel))
    if act_func_name:
        if single_act_channel:
            seq.add_module(f"{act_func_name}{postfix}",
...
def single_conv(in_channel, out_channel, kernel_size=3, stride=1, padding=1, dilation=1,
                groups=1, bias=True, seq=None, act_func_name=None, norm_func_name=None, single_act_channel=False):
    if norm_func_name in ['batch_norm_2d', 'instance_norm_2d']:
        bias = False
    if seq is None:
        seq = nn.Sequential()
    seq.add_module(f"conv{kernel_size}x{kernel_size}s{stride}g{groups}",
                   nn.Conv2d(in_channel, out_channel, kernel_size,
                             stride, padding, dilation, groups, bias=bias))
    seq = norm_and_act(out_channel, act_func_name, norm_func_name,  single_act_channel, seq=seq)
    return seq


def dual_conv(in_channel, mid_channel, out_channel, strides=[1, 1], bias=True,
              seq=None, act_func_name=None, norm_func_name=None, single_act_channel=False):
    if seq is None:
        seq = nn.Sequential()
    ...
    seq = norm_and_act(out_channel, act_func_name, norm_func_name,  single_act_channel, seq=seq, postfix="_1")
    seq.add_module("conv2", nn.Conv2d(
        mid_channel, out_channel, kernel_size=3, stride=strides[1], padding=1, bias=bias))
    seq = norm_and_act(out_channel, act_func_name, norm_func_name,  single_act_channel, seq=seq, postfix="_2")
    return seq


class GhostModule(nn.Module):
    def __init__(self, in_channel, out_channel, primary_kernal_size=1, cheap_kernel_size=3, act_func_name=None, norm_func_name=None):
        super().__init__()
        assert out_channel % 2 == 0
        hidden_channel = out_channel // 2
        self.primary_conv = single_conv(in_channel, hidden_channel, kernel_size=primary_kernal_size, padding=0,
                                        act_func_name=act_func_name, norm_func_name=norm_func_name)
        self.cheap_conv = single_conv(hidden_channel, hidden_channel, kernel_size=cheap_kernel_size,
                                      padding=cheap_kernel_size//2, groups=hidden_channel,
                                      act_func_name=act_func_name, norm_func_name=norm_func_name)

    def forward(self, input_layer):
        primary = self.primary_conv(input_layer)
        return torch.cat([primary, self.cheap_conv(primary)], dim=1)
...
class InputBlock(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel, act_func_name, norm_func_name=None):
        ...
        self.seq = dual_conv(in_channel, mid_channel, out_channel,
                             seq=self.seq, act_func_name=act_func_name, norm_func_name=norm_func_name)
...
class EncoderBlock(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel, act_func_name, norm_func_name=None):
        super().__init__()
        self.seq = nn.Sequential()
        self.seq = dual_conv(in_channel, mid_channel,
                             out_channel, strides=[2, 1], seq=self.seq, act_func_name=act_func_name, norm_func_name=norm_func_name)
...
class DecoderBlock(nn.Module):
    def __init__(self, in_channel, concat_channel, mid_channel, out_channel, act_func_name=None, norm_func_name=None):
        ...
        self.seq = nn.Sequential()
        self.seq = dual_conv(in_channel + concat_channel, mid_channel,
                             mid_channel, seq=self.seq, act_func_name=act_func_name, norm_func_name=norm_func_name)
...
class UNetEncoder(Inputable):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        # log.debug(dict_to_string([self.in_channel]))
        ...
        if 'skip_layer_split' in self.config.keys():
            assert self.n_encoder-1 == len(self.config['skip_layer_split']), f'len(self.config["skip_layer_split"]) = {len(self.config["skip_layer_split"])}\
should be same as n_encoder = {self.n_encoder}, when enabling "skip_layer_split"'
        self.encoders_skip_channel = []
    ...
    def init_encoder_layers(self):
        self.encoders_skip_channel.append(self.last_channel)
        self.encoders = nn.ModuleList()
        class_list = self.config['struct'].get('encoder_class', None)
        for i in range(self.n_encoder):
            ...
            encoder_block_cfg = {'in_channel': self.last_channel,
                         'mid_channel': mid_channel,
                         'out_channel': out_channel,
                         'act_func_name': self.act_func_name,
                         'norm_func_name': self.norm_func_name,
                         'index': i,
                         'num_enc': self.n_encoder
                         }
            if class_list is not None:
                encoder_block_cfg.update({'class': class_list[i]})
            self.encoders.append(self.create_encoder_block(encoder_block_cfg))
            if i < self.n_encoder - 1 and 'skip_layer_split' in self.config.keys():
                assert self.config['skip_layer_split'][i] <= out_channel
                self.encoders_skip_channel.append(self.config['skip_layer_split'][i])
            else:
                self.encoders_skip_channel.append(out_channel)
            self.last_channel = out_channel
        self.config['encoders_skip_channel'] = self.encoders_skip_channel

    def forward(self, encoding):
        cur_input = encoding
        sc_layers = [__space_holder__ for _ in range(len(self.encoders)+1)]

        if self.input_block is not None:
            cur_input = self.input_block(cur_input)
            if self.config['skip-layer']:
                sc_layers[0] = cur_input
        ...
        for i in range(len(self.encoders)):
            ...
            if self.config['skip-layer'] and i != len(self.encoders) - 1:
                ...
                sc_layers[i+1] = sc_layer
            else:
                sc_layers[i+1] = cur_input
        return sc_layers
...
class UNetDecoder(Outputable):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.output_prefix = self.config.get('output_prefix', "")
        self.encoders_skip_channel = self.config['encoders_skip_channel']
        self.last_channel = 0
        ...
        ''' layer_output won't be concated to next layer '''
    ...
    def init_decoder_layers(self) -> None:
        ...
        self.last_channel = self.encoders_skip_channel[-1]
        self.last_channel += self.config['struct'].get("concat", 0)
        class_list = self.config['struct'].get('decoder_class', None)
        for i in range(self.n_decoder):
            ...
            if i == self.n_decoder - 1 and 'output' not in self.config['struct'].keys():
                ''' merge channels of output block with last decoder block ''' 
                out_channel += self.out_channel
            if i == self.n_decoder - 1:
                self.decoder_layer_out_channel = out_channel
            concat_channel = 0
            if self.config['skip-cat'] and i >= self.config['skip_conn_start']:
                ...
                concat_channel += self.encoders_skip_channel[-(i + self.config['skip_conn_offset'])]
            ...
            decoder_block_cfg = {'in_channel': self.last_channel - layer_output_channel,
            ...
            if class_list is not None:
                decoder_block_cfg.update({'class': class_list[i]})
            self.decoders.append(self.create_decoder_block(decoder_block_cfg))
            self.last_channel = out_channel
    ...
    def init_output_layer(self) -> None:
        ...
        if len(self.encoders_skip_channel) > self.n_decoder:
            concat_channel = self.encoders_skip_channel[-self.n_decoder-1]
        else:
            concat_channel = 0
        self.output_block = self.create_output_block(
            {'in_channel': self.last_channel + concat_channel,
             'mid_channel': mid_channel,
             'out_channel': out_channel,
             'act_func_name': self.act_func_name,
             'norm_func_name': self.norm_func_name,
