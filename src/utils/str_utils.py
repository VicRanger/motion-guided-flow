...
def check_type(lst, typ):
    return all(isinstance(item, typ) for item in lst)

def dict_to_string(d, k='', depth=0, max_depth=0, mmm=False, full_name=True):
    displayed = False
    if depth == 0:
        max_depth = max(1, get_dict_depth(d))
    ret = ""
    k_str = (str(k) if k != None else '')
    assert 64 + (max_depth - depth) * 8 > 0, f"max_depth: {max_depth}, depth: {depth}"
    formatter = "{{}}{{:<{}}} {{:<{}}} {{:>{}}}".format(
    ...
    if isinstance(d, torch.Tensor):
        ...
        elif d.dtype == torch.float64 or d.dtype == torch.float32 or d.dtype == torch.float16 or d.dtype == torch.uint8 or d.dtype == torch.int64:
        ...
        if dim == 0:
            ret = formatter.format(
                '\t' * int(depth), k_str, "dim:{}, {}".format(dim, type_str), d)
        else:
            ret = formatter.format(
                '\t' * int(depth), k_str, "dim:{}, {}".format(dim, type_str), "")
        ...
        if len(d) == 0:
            ...
            if (check_type(d, numbers.Number) or check_type(d, str)):
                ret = formatter.format('\t' * depth, k_str, str(type(d)), str(d))
                displayed = True
            # elif not isinstance(d[0], list) and not isinstance(d[0], dict):
            #     ret = formatter.format(
            #         '\t' * depth, k_str, str(type(d))+f"({len(d)})", "[0].type: {}".format(str(type(d[0]))))
                # displayed = True
        ...
        if type(d).__str__ is not object.__str__:
            ret = formatter.format('\t' * depth, k_str, str(d), "")
        else:
